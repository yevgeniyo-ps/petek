-- ============================================
-- Petek: Add last_activity_at to admin_get_users
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(ch.cnt, 0) AS challenges_count,
  ...
  LEFT JOIN (
    SELECT user_id, count(*) AS cnt, sum(pg_column_size(notes.*)) AS sz,
      ...
      max(GREATEST(notes.created_at, notes.updated_at)) AS last_at
    FROM notes GROUP BY user_id
  ) n ON n.user_id = u.id
  LEFT JOIN (
    SELECT user_id, count(*) AS cnt, sum(pg_column_size(collections.*)) AS sz,
      max(GREATEST(collections.created_at, collections.updated_at)) AS last_at
  ...
  LEFT JOIN (
    SELECT user_id, count(*) AS cnt, sum(pg_column_size(insurance_policies.*)) AS sz,
      ...
      max(insurance_policies.created_at) AS last_at
    FROM insurance_policies GROUP BY user_id
  ) p ON p.user_id = u.id
  LEFT JOIN (
    SELECT user_id, count(*) AS cnt, sum(pg_column_size(challenges.*)) AS sz,
      max(challenges.created_at) AS last_at
    FROM challenges GROUP BY user_id
  ...
  LEFT JOIN (
    SELECT cp2.user_id, sum(pg_column_size(cp2.*)) AS sz
